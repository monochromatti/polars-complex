...
class ComplexArithmetic:
    ...
    @staticmethod
    def struct(real: pl.Expr | str, imag: pl.Expr | str):
        if isinstance(real, str):
            real = pl.col(real)
        if isinstance(imag, str):
            imag = pl.col(imag)
        name = real.meta.output_name().strip(".real")
        return pl.struct(real.alias("real"), imag.alias("imag")).alias(f"{name}[c]")
    ...
    def conj(self):
        real, imag = self._explode()
        return self.struct(real, -imag).alias(self._name)
    ...
    def inverse(self):
        real, imag = self._explode()
        denom = real.pow(2) + imag.pow(2)
        return self.struct(real / denom, -imag / denom).alias(self._name)
    ...
    def relative_difference(self, expr):
        ...
        return self.struct(real - 1, imag)
